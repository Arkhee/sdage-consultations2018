...
if(isset($cmdOk) && $myCodeImport!="")
{
	if(isset($userfile) && is_readable($userfile["tmp_name"]))
	{
		...
		{
			$myObj->libelle_import_description="Import Données Rendu Labos";
			$myObj->auteur_import_description="YBS";
			$myObj->firstline_import_description=IMPORTRENDU_FIRST_ROW;
			$myObj->headerline_import_description=IMPORTRENDU_HEADER_ROW;
			$myObj->firstcolumn_import_description=IMPORTRENDU_FIRST_COL;
			$myObj->date_modification_import_description=date("Y-m-d");
			$myObj->description_import_description=$_FILES["userfile"]["name"]." => Import conçu pour récupération des données labo selon fichier de rendu";
			...
			for ($row=1;$row<$xls->rowcount();$row++)
			{
				...
				*/
				...
				if($return && $myObj->type_import_colonne=="analyse")
				{
					...
					{
						echo "Ajout lien entre colonne et paramètre : ".$xls->val($row,DESCR_COL_CODESANDRE)."/".$myCodeParamRef." => ".$myObjId."<br />\n";
						$myRel=new stdClass();
