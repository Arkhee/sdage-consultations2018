   $this->products[$index] = array('pid' => $orders_products['products_id'],
		...
		//debut modif by paddybl for ecotax
								'ecotax_rates_id' => $orders_products['ecotax_rates_id'],
								'products_ecotax' => $orders_products['products_ecotax'],
...
//debut modif by paddybl for ecotax
			...
			$this->info['ecotax_groups']["$products_ecotax_description"] +=tep_get_ecotax_taxable($this->products[$index]['ecotax_rates_id'],$shipping_address['entry_country_id'],$shipping_address['entry_zone_id'])*$this->products[$index]['qty'];
          } else {
            $this->info['tax_groups']["$products_tax_description"] = $shown_price - ($shown_price / (($products_tax < 10) ? "1.0" . str_replace('.', '', $products_tax) : "1." . str_replace('.', '', $products_tax)))+tep_get_ecotax_taxable($this->products[$index]['ecotax_rates_id'],$shipping_address['entry_country_id'],$shipping_address['entry_zone_id'])*$this->products[$index]['qty'];
			$this->info['ecotax_groups']["$products_ecotax_description"] +=tep_get_ecotax_taxable($this->products[$index]['ecotax_rates_id'],$shipping_address['entry_country_id'],$shipping_address['entry_zone_id'])*$this->products[$index]['qty'];
          }
        } else {
          $this->info['tax'] += ($products_tax / 100) * $shown_price+tep_get_ecotax_taxable($this->products[$index]['ecotax_rates_id'],$shipping_address['entry_country_id'],$shipping_address['entry_zone_id'])*$this->prBinLogName = 'Protokoll';
