...
class Spreadsheet_Excel_Writer_Worksheet extends Spreadsheet_Excel_Writer_BIFFwriter
{
    ...
    */
    function write($row, $col, $token, $format = null,$tokenlabel="")
    {
        // Check for a cell reference in A1 notation and substitute row and column
        /*if ($_[0] =~ /^\D/) {
            @_ = $this->_substituteCellref(@_);
    }*/
		if(isset($format->_num_format))
		{
			switch($format->_num_format)
			{
				case "@":
				...
				case "url":
					if (preg_match("/^[fh]tt?p:\/\//", $token))
					    return $this->writeUrl		($row, $col, $token, $tokenlabel, $format);
			        else
						return $this->writeString	($row, $col, $token, $format);
					break;
				case "jj/mm/aaaa":
