...
if(isset($cmdOk) && $myCodeImport!="")
{
	if(isset($userfile) && is_readable($userfile["tmp_name"]))
	{
		...
		if($database->loadObject($myObj))
		{
			...
			$myObj->firstline_import_description="".strval(IMPORT_FIRST_ROW);
			$myObj->headerline_import_description="".strval(IMPORT_HEADER_ROW);
			$myObj->firstcolumn_import_description="".strval(IMPORT_FIRST_COL);
			$myObj->date_modification_import_description=date("Y-m-d");
			$myObj->description_import_description=$_FILES["userfile"]["name"]." => Import conçu pour récupération des données poissons du fichier DIREN";
