...
define('AIRVIRO_HOUR_FORMAT', 'H:i:s');
...
class Spreadsheet_Excel_Reader {
	...
	function rawval($row,$col,$sheet=0) {
		$col = $this->getCol($col);
		if (array_key_exists($row,$this->sheets[$sheet]['cellsInfo']) && array_key_exists($col,$this->sheets[$sheet]['cellsInfo'][$row])) {
			return print_r($this->sheets[$sheet]['cellsInfo'][$row][$col],1);
		}
		return "";
	}
			...
			switch ($code) {
				 ...
				 */
                    ...
                    if ((ord($this->data[$spos+6])==0) &&(ord($this->data[$spos+12])==255) && (ord($this->data[$spos+13])==255)) {
                        //String formula. Result follows in a STRING record
//mmptext begin                        
                        //echo "FORMULA $row $column Formula with astring<br>\n";
...
//mmptext end                        
                        ...
                        // result is a number, so first 14 bytes are justlike a _NUMBER record
                        $tmp = unpack("ddouble", substr($this->data, $spos+ 6, 8)); // It machine machine dependent
                        if ($this->isDate($spos)) {
                         ...
                         //   $this->addcell(DateRecord($r, 1));
                            ...
                            $string="";
                            if(isset($this->curformat) && isset($this->multiplier))
            ...
            //echo "In STRING section, BIFF8\n<br>";
                        $xpos = $spos;
                                                $numChars =ord($this->data[$xpos]) | (ord($this->data[$xpos+1]) << 8);
                                                $xpos += 2;
                                                $optionFlags =ord($this->data[$xpos]);
                                                $xpos++;
                                        $asciiEncoding = (($optionFlags &0x01) == 0) ;
                                                $extendedString = (($optionFlags & 0x04) != 0);

                                                // See if string containsformatting information
                                                $richString = (($optionFlags & 0x08) != 0);

                                                if ($richString) {
                                        // Read in the crun
                                                        $formattingRuns =ord($this->data[$xpos]) | (ord($this->data[$xpos+1]) << 8);
                                                        $xpos += 2;
                                                }
                                                ...
                                                $len = ($asciiEncoding)?$numChars : $numChars*2;
                                                $retstr =substr($this->data, $xpos, $len);
                                                $xpos += $len;
               ...
               // echo "In STRING section, BIFF7\n<br>";
                        $xpos = $spos;
                                                $numChars =ord($this->data[$xpos]) | (ord($this->data[$xpos+1]) << 8);
                                                $xpos += 2;
                                                $retstr =substr($this->data, $xpos, $numChars);
                                                echo "Str = $retstr\n<br>";
                    }
